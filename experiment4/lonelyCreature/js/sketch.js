...
function setup() {
    ...
    let canvas = createCanvas(800, 600); // Set initial size
    ...
    // Create the "World key" label
    let label = createP(); // Create a paragraph element
    label.html("World key: "); // Set label text
    label.parent("container"); // Add to the container div

    // Create the input field for the world key
    let userInput = createInput("xyzzy"); // Initial key value
    userInput.parent(label); // Nest input inside the label
    userInput.input(() => {
        rebuildWorld(userInput.value()); // Rebuild the world when the input changes
    });

    // Add instructions below the canvas
    createP("Arrow keys scroll. Clicking changes tiles.").parent("container");
    rebuildWorld(userInput.value());
